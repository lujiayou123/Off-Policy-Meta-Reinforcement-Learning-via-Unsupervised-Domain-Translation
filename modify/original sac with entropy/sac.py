...
class PEARLSoftActorCritic(MetaRLAlgorithm):
    ...
    def _take_step(self, indices, context):
        ...
        with torch.no_grad():
            ...
            min_qf_next_target = torch.min(q1_next_target, q2_next_target) - self.alpha * log_pi#计算较小的target Q         line 65
            next_q_value = rewards_flat + (1. - terms_flat) * self.discount * min_qf_next_target#q=r+(1-d)γ(Vst+1)         line 66
        q1, q2 = self.critic(obs, actions, task_z)  # forward                                                           line 68
        q1_loss = F.mse_loss(q1, next_q_value)#JQ = 𝔼(st,at)~D[0.5(Q1(st,at) - r(st,at) - γ(𝔼st+1~p[V(st+1)]))^2]      line 69
        ...
        q1_pi, q2_pi = self.critic(obs, pi, task_z.detach())  # 动作的Q值                                                        line 74
